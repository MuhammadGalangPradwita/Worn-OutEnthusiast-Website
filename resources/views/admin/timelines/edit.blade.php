...
@section('content')
    <div class="max-w-2xl">
        <div class="flex items-center gap-4 mb-6">
            ...
            <h2 class="font-display font-bold text-xl text-white">Edit Milestone</h2>
        </div>
        ...
        <form action="{{ route('admin.timelines.update', $timeline) }}" method="POST"
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $timeline->title) }}" required
            ...
            <div>
                ...
                <input type="text" name="date" value="{{ old('date', $timeline->date) }}" required
                    class="w-full bg-denim-700/30 border border-denim-600/30 rounded-lg px-4 py-3 text-white text-sm focus:outline-none focus:border-denim-400 transition-colors">
            </div>
            <div>
                <label class="block text-sm text-denim-300 mb-2">Sort Order</label>
                <input type="number" name="sort_order" value="{{ old('sort_order', $timeline->sort_order) }}"
                    class="w-full bg-denim-700/30 border border-denim-600/30 rounded-lg px-4 py-3 text-white text-sm focus:outline-none focus:border-denim-400 transition-colors">
            </div>
            <div class="flex items-center gap-3">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ $timeline->is_active ? 'checked' : '' }}
